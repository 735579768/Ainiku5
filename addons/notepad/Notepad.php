...
if (!class_exists('\addons\notepad\Notepad')) {
	class Notepad extends \app\common\controller\Addon {
		...
		public function gettree($pid = 0) {
			if ($pid === 0) {return '';}
			// $info          = M('PluginNotepads')->field('name')->find($pid);
			$info          = $this->getParam();
			$map['pid']    = $pid;
			$map['status'] = 1;
			$list          = \think\Db::name('AddonNotepad')->where($map)->order('create_time desc')->select();
		...
		public function add($arc_id = 0) {
			...
			if (request()->isPost()) {
				...
				include SITE_PATH . '/addons/notepad/validate/Notepad.php';
				...
				$vali = new \addons\notepad\validate\Notepad();
				if ($vali->check($data)) {
					$data['url'] = preg_replace('/http\:\/\//i', '', $data['url']);
					cookie('notepad_name', $name, 3600 * 24 * 365);
					cookie('notepad_email', $email, 3600 * 24 * 365);
					cookie('notepad_url', $url, 3600 * 24 * 365);
					$result = \think\Db::name('AddonNotepad')->insertGetId($data);
					if (0 < $result) {
						$data['notepad_id'] = $result;
						$list[]             = $data;
						$this->assign('_list', $list);
		...
		private function sendemail() {
			...
			//给被回复的人发邮件
			if ($pid != 0) {
				$rows = M('PluginNotepads')->field('name,arc_id,email,email_notify')->find($pid);
