...
{
  var i=0;
 $(".hex").removeAttr("data-clipboard-text");
  if(bgColor.substring(0,3)=="rgb") {
    bgColor = bgColor.substring(4,bgColor.length-1);
    var bgColor2 = bgColor.replace(/[ ]/g,"");
    rgbArray = bgColor2.split(",");
...
{
	if (r>255){
		 alert("要求0 和 255之间的整数。已插入最接近的数值。");
		 r=255;
		 } 
	if (g>255) {
		 alert("要求0 和 255之间的整数。已插入最接近的数值。");
...
function DrawWheel()
{
  ...
  for (i = 0; i < 12; i++) 
	{
		...
		document.getElementById(i+'HEX').innerHTML="#" + red + green + blue+"<div class='tishi'></div>";
		document.getElementById(i+'RGB').innerHTML=tempRGB.R+"."+tempRGB.G+"."+tempRGB.B;
		
		document.getElementById(i+'HEX').setAttribute("data-clipboard-text", red + green + blue);
		HSL.H+=30;
		if (HSL.H>=360)
			HSL.H-=360;
	}
  ...
  for (i = 1; i <= 4; i++) 
    ...
    HSLtoRGB(HSL.H,HSL.S,HSL.L);
		...
		document.getElementById('m'+i+'HEX').innerHTML="#" + red + green + blue + "<div class='tishi'></div>";		
		document.getElementById('m'+i+'RGB').innerHTML=tempRGB.R+"."+tempRGB.G+"."+tempRGB.B;
		 
		document.getElementById('m'+i+'HEX').setAttribute("data-clipboard-text", red + green + blue);
	}
}
  ...
  for (outer = 255; outer >= 0; outer-=51)
  {
    for (middle = 255; middle >= 0; middle-=51)
    {
      for (inner = 255; inner >= 0; inner-=51)
      {
        ...
        document.write('<td bgcolor="#'+tR+tG+tB+'" class="click_rgb"><a href="#" data="'+tR+tG+tB+'" onClick="ChangeColors('+(255 - outer)+','+(255 - middle)+','+(255 - inner)+'); return false;"><div class="img"></div></a></td>');
      }
    }
